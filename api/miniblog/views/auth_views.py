...
def api_login():
    ...
    if not user or not user.credentials or not check_password_hash(user.credentials.password_hash, password):
...
def register_user():
    ...
    try:
        new_user = Usuario(nombre=username, correo=email, role=role)
    ...
    return jsonify({
        ...
        "usuario": { "id": new_user.id, "username": new_user.nombre, "email": new_user.correo, "role": new_user.role }
    }), 201
